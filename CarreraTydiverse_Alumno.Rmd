...
```{r}
cran
glimpse(cran)#Info en forma de vector
```
# dplyr
...
```{r}
avg_bytes <- cran %>% summarise(avr_bytes = mean(size))
avg_bytes
```
# Agrupamientos
...
by_package
ungroup(by_package)#Se quitan los groups
...
#Te da el valor medio por cada paquete
```
...
pack_sum <- by_package %>% summarise(avg_size = mean(size), 
                         count = n(), 
                         unique = n_distinct(ip_id),
                         countries = n_distinct(country),
...
top_unique_sorted <-  cran %>% group_by(package) %>% summarise(unique =  n_distinct(ip_id))
...
cran %>%  mutate(size_mb = size / 2^20 ) %>% filter(size_mb <= 0.5) %>% arrange(desc(size_mb))
